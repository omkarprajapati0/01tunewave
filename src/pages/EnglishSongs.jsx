...
import { englishSongs as localEnglishSongs } from "../data/allSongs";
...
export default function EnglishSongs() {
  ...
  const loadLocalSongs = useCallback(
    (loadMore = false) => {
      ...
      const allShuffled = shuffleArray(localEnglishSongs);
      ...
      setLocalHasMore(
        currentOffset + SONGS_PER_PAGE < localEnglishSongs.length,
      );
  ...
  const songs = isSpotifyConfigured ? apiEnglishSongs : displayedSongs;
  const displaySong =
    currentSong || (songs.length > 0 ? songs[0] : localEnglishSongs[0]);

  // Load more songs when scrolling - handles both API and local
  const loadMore = useCallback(() => {
    if (isSpotifyConfigured) {
      // Use API loading
      if (!loadingCategories.english && hasMore.english) {
        fetchEnglishSongs(20, offsets.english, true);
      }
    } else {
      // Use local loading
  ...
  useInfiniteScroll(loadMore, {
    enabled: isSpotifyConfigured
      ? hasMore.english && !loadingCategories.english
      : localHasMore && !isLoadingLocal,
    isLoading: isSpotifyConfigured ? loadingCategories.english : isLoadingLocal,
  ...
  useEffect(() => {
    if (isSpotifyConfigured && apiEnglishSongs.length === 0) {
      fetchEnglishSongs(20, 0, false);
    }
  }, [isSpotifyConfigured, apiEnglishSongs.length, fetchEnglishSongs]);
  ...
  const handleRefresh = () => {
    ...
    if (isSpotifyConfigured) {
      ...
      fetchEnglishSongs(20, 0, false).then(() => {
  ...
  const performSearch = useCallback(
    async (query) => {
      ...
      const localResults = localEnglishSongs.filter(
        (song) =>
  ...
  const handleDownloadCurrentSong = async () => {
    ...
    try {
      console.log("EnglishSongs: Watch video triggered for", song.title);
      ...
      console.error("EnglishSongs: Watch video error:", error);
      showDownloadStatus(
        ...
        <header className="top-nav">
          ...
          <nav>
            ...
            <button
              ...
              disabled={loadingCategories.english || isLoadingLocal}
              title="Refresh songs"
              style={{
                background:
                  loadingCategories.english || isLoadingLocal
                    ? "rgba(0,0,0,0.1)"
                    : "transparent",
                borderRadius: "50%",
                width: "40px",
            ...
            >
              <i
                className={`fa-solid ${loadingCategories.english || isLoadingLocal ? "fa-spinner fa-spin" : "fa-refresh"}`}
              ></i>
            </button>
            <Link
              to="/notifications"
        ...
        {showSearch && (
          <div className="search-container mb-6" style={{ padding: "0 20px" }}>
            <div className="relative">
              <input
                type="text"
                placeholder="Search English songs..."
                value={searchQuery}
        ...
        <div className="panel" style={{ marginTop: "10px" }}>
          <div className="section-header">
            <h3 style={{ color: "#222", fontSize: "22px" }}>
              {showSearch ? "Search Results" : "English Songs :~"}
            </h3>
          ...
          {loadingCategories.english && songs.length > 0 && (
            <div
          ...
          {!hasMore.english && songs.length > 0 && isSpotifyConfigured && (
            <div
              style={{ textAlign: "center", padding: "20px", color: "#888" }}
            >
